var missingMega4 = function() {
  ...
  if (dlg.length == 0 || !dlg.dialog('isOpen')) {
    dlg.remove();
    dlg = $("<div id='missingMega4' title='MEGA4 not found'><p>MEAG4 has USB device ID \"2109:0817\". However this ID is not listed when running \"lsusb\".</p><p>Please check the physical connection between MEGA4 and Orange Pi.</p></div>");
    dlg.dialog({
      autoOpen: true,
      closeOnEscape: false,
      height: 160,
...
var missingSoftware = function() {
  ...
  if (dlg.length == 0 || !dlg.dialog('isOpen')) {
    dlg.remove();
    dlg = $("<div id='MissingSoftware' title='MEGA4 software missing'><p>MEGA4's software should be installed beforehand.</p><p>Usually it is installed to the \"/home/orangepi/mega4\" directory. If it was installed to a different location, please configure accordingly in the \"uwi.conf\" file.</p></div>");
    dlg.dialog({
      autoOpen: true,
      closeOnEscape: false,
      height: 160,
...
var updateDevice = function(device) {
  ...
  var k4 = dev.find('.k4');
  updateStates(device, d11, d1, k1, d12, d2, k2, d13, d3, k3, d14, d4, k4);
  var conn = (device[0].indexOf('.') === -1) ? 'to OPi' : 'to ' + device[0].slice(0, device[0].lastIndexOf('.'));
